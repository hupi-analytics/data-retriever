...
SimpleCov.profiles.define "root_filter" do
  ...
  add_filter do |src|
    begin
      ...
      puts 'hello'
      p [Encoding.default_external, Encoding.default_internal, Encoding.find('locale'), Encoding.find('filesystem'), __ENCODING__, src.filename.encoding, src.filename.valid_encoding?, SimpleCov.root.to_s.encoding, SimpleCov.root.to_s.valid_encoding?, Encoding.compatible?(src.filename, SimpleCov.root)]
      puts "Warning: Encoding::CompatibilityError"
      puts "skip file : #{src.filename}"
      false
    rescue Encoding::UndefinedConversionError
      p [Encoding.default_external, Encoding.default_internal, Encoding.find('locale'), Encoding.find('filesystem'), __ENCODING__, src.filename.encoding, src.filename.valid_encoding?, SimpleCov.root.to_s.encoding, SimpleCov.root.to_s.valid_encoding?, Encoding.compatible?(src.filename, SimpleCov.root)]
      puts "Warning: Encoding::UndefinedConversionError"
      puts "skip file : #{src.filename}"
      false
    end
  end
